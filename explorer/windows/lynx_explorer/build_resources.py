...
try:
    ...
    try:
        ...
        showcase_build_dir = os.path.join(root_dir, 'explorer', 'showcase', 'dist')
        ...
        # Copy all files from showcase build directory to Windows showcase directory
        if os.path.exists(showcase_build_dir):
            for item in os.listdir(showcase_build_dir):
                src_item = os.path.join(showcase_build_dir, item)
                dst_item = os.path.join(windows_showcase_dir, item)
