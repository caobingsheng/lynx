...
std::tuple<int32_t, float, float> UIList::CalcSnapScroll(bool forward,
  ...
  float scroll_range = GetScrollRange();
  ...
  for (UIBase* child : children_) {
    ...
    if (IsListItem(child) && HasParentDrawNode(child)) {
      ...
      if (vertical ? IsVisibleCellVertical(list_item)
        ...
        if (item_snap_offset > scroll_range) {
          clamped_item_snap_offset = scroll_range;
          // Consider child's item_snap_offset may be clamped, here we choose
          // the child which has the min item_snap_offset.
